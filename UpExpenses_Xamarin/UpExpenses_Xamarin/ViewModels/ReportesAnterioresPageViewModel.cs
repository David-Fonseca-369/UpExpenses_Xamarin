...
{
    public class ReportesAnterioresPageViewModel : BaseViewModel
    {
        ...
        public ReportesAnterioresPageViewModel(INavigation navigation)
        {
            ...
            ItemTappedCommand = new Command<CardReportePendiente>((item) =>
            {
                ...
                navigation.PushAsync(new DetallesReporteAnteriorPage());
            });
            ...
            Add = new Command(async () => await App.Current.MainPage.Navigation.PushAsync(new NuevoReporteGastosPage()));

            RefreshCommand = new Command(() =>
            {
                Task.Run(async () => 
                {
                    await WebServiceFunctions.SendPendingsReports();
                    await LoadReports();
                });
                IsRefreshing = false;
        ...
        private async Task<bool> LoadReports()
        {
            try
            {
                IList<CardReportePendiente> result = await App.DataBase.GetAllReportsPreviousCard((int)UserData.Emp_Num);
