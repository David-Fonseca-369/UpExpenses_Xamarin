...
{
    public class ReportesEnviadosPageViewModel : BaseViewModel
    {
        ...
        public ReportesEnviadosPageViewModel(INavigation navigation)
        {
            ...
            ItemTappedCommand = new Command<CardReportePendiente>((item) =>
            {
                ...
                navigation.PushAsync(new DetallesReporteEnviadoPage());
            });
        ...
        private async Task<bool> LoadReports()
        {
            try
            {
                IList<CardReportePendiente> result = await App.DataBase.GetAllReportsSentCard((int)UserData.Emp_Num);
